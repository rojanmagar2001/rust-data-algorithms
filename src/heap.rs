...
//! min-heap or a max-heap. It supports commin heap operations such as adding,
...
/// allowing for both min-heap and max-heap functionalities, as well as custom sorting.
pub struct Heap<T> {
    items: Vec<T>,
...
impl<T> Heap<T> {
    ...
    pub fn from_vec(itmes: Vec<T>, comparator: fn(&T, &T) -> bool) -> Self {
        let heap = Self { items, comparator };
        heap.build_heap();
        heap
    }

    /// Constructs from the heap from an unsorted vector by applying the heapify process.
